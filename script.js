...
function updateCartUI() {
  ...
  if (tot) tot.textContent = `₱${total + delivery}`;
}
...
function updateOrderType(radio) {
  ...
  if (ag) ag.style.display = deliveryType === "delivery" ? "block" : "none";
}
...
function proceedToForm() { closeGcashModal(); openCheckout(); }
...
async function placeOrder() {
  ...
  if (phoneVal.length < 10) { alert("Please enter a valid 10 or 11-digit phone number."); return; }
...
// ============================================
let lbScale = 1, lbX = 0, lbY = 0, lbDragging = false, lbLastX = 0, lbLastY = 0, lbPinchDist = null;
...
function openImageModal(src) {
  ...
  if (!document.getElementById("zoomControls")) {
    ...
    controls.innerHTML = `<button onclick="lbZoom(-0.25)">−</button><span class="zoom-level" id="zoomLevel">100%</span><button onclick="lbZoom(0.25)">+</button><button onclick="lbReset()">↺</button>`;
...
function lbZoom(delta) {
  ...
  applyLbTransform(); updateZoomLevel();
}

function lbReset() { lbScale = 1; lbX = 0; lbY = 0; applyLbTransform(); updateZoomLevel(); }

document.addEventListener("DOMContentLoaded", () => {
  const img = document.getElementById("lightboxImage");
  if (!img) return;

  img.addEventListener("wheel", (e) => { e.preventDefault(); lbZoom(e.deltaY < 0 ? 0.15 : -0.15); }, { passive: false });

  img.addEventListener("mousedown", (e) => {
    if (lbScale <= 1) return;
    e.preventDefault(); lbDragging = true; lbLastX = e.clientX; lbLastY = e.clientY; img.classList.add("dragging");
  });
  document.addEventListener("mousemove", (e) => {
    if (!lbDragging) return;
    lbX += e.clientX - lbLastX; lbY += e.clientY - lbLastY;
    lbLastX = e.clientX; lbLastY = e.clientY; applyLbTransform();
  });
  document.addEventListener("mouseup", () => { lbDragging = false; img.classList.remove("dragging"); });

  img.addEventListener("touchstart", (e) => {
    if (e.touches.length === 2) {
      lbPinchDist = Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY);
    } else if (e.touches.length === 1 && lbScale > 1) {
      lbDragging = true; lbLastX = e.touches[0].clientX; lbLastY = e.touches[0].clientY;
    }
    e.stopPropagation();
  }, { passive: true });
  ...
  img.addEventListener("touchmove", (e) => {
    if (e.touches.length === 2 && lbPinchDist !== null) {
      ...
      lbScale = Math.min(5, Math.max(0.5, lbScale + (newDist - lbPinchDist) * 0.01));
      lbPinchDist = newDist; applyLbTransform(); updateZoomLevel();
    } else if (e.touches.length === 1 && lbDragging) {
      lbX += e.touches[0].clientX - lbLastX; lbY += e.touches[0].clientY - lbLastY;
      lbLastX = e.touches[0].clientX; lbLastY = e.touches[0].clientY; applyLbTransform();
    }
    e.stopPropagation();
  }, { passive: true });

  img.addEventListener("touchend", (e) => { if (e.touches.length < 2) lbPinchDist = null; if (e.touches.length === 0) lbDragging = false; });

  img.addEventListener("dblclick", (e) => {
    e.stopPropagation();
    if (lbScale > 1) { lbReset(); } else { lbScale = 2.5; applyLbTransform(); updateZoomLevel(); }
  });
});
...
async function fetchReviewsFromDB() {
  ...
  } catch (e) { console.error("Could not fetch reviews from DB", e); }
  renderReviews();
}
...
function renderReviews() {
  ...
  grid.innerHTML = reviews.map(rev => `
    <div class="review-card" style="scroll-snap-align: start;">
      ...
      <div style="color: #FACC15; font-size: 1.2rem; margin-bottom: 10px;">${'★'.repeat(rev.stars)}${'☆'.repeat(5 - rev.stars)}</div>
...
function scrollReviews(direction) {
  ...
  if (container) container.scrollBy({ left: direction * 324, behavior: 'smooth' });
}

function initStarRating() {
  const stars = document.querySelectorAll("#starRating span");
  if (stars.length === 0) return;
  stars.forEach(s => { s.style.cursor = "pointer"; s.style.fontSize = "1.8rem"; s.style.color = "#FACC15"; });
  stars.forEach(star => {
    star.addEventListener("click", function() {
      currentRating = parseInt(this.getAttribute("data-val"));
      stars.forEach((s, index) => { s.style.color = index < currentRating ? "#FACC15" : "#E5E7EB"; });
    });
  });
}
...
async function submitReview() {
  ...
  if (!nameInput || !textInput) { alert("Please enter both your name and a review."); return; }

  const newReview = {
    name: nameInput, stars: currentRating,
    date: new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
    text: textInput, avatar: nameInput.charAt(0).toUpperCase()
  };

  try {
    await fetch(API_URL + '/api/reviews', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(newReview) });
...
function sendMessage() {
  ...
  if (!name || !email || !body) { alert("Please fill out all fields before sending."); return; }
  ...
  form.method = "POST"; form.action = "https://formsubmit.co/resquiciocarl1@gmail.com"; form.target = "_blank";

  const addInput = (n, v) => { const i = document.createElement("input"); i.type = "hidden"; i.name = n; i.value = v; form.appendChild(i); };
  addInput("name", name); addInput("email", email); addInput("message", body);
  addInput("_subject", `New Paresalamat Message from ${name}`);
  addInput("_captcha", "false"); addInput("_template", "table");
...
(function initReelControls() {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    progWrap.addEventListener("touchstart", e => { seek(e.touches[0]); e.preventDefault(); }, { passive: false });
    progWrap.addEventListener("touchmove", e => { seek(e.touches[0]); e.preventDefault(); }, { passive: false });
